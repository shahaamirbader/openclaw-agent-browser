...
> **Based on [vercel-labs/agent-browser](https://github.com/vercel-labs/agent-browser)** — this fork adds features for deeper AI agent integration, including screenshot base64 passthrough, cursor position overlays, form state extraction, and WebSocket message inspection. Full credit to the original authors at Vercel Labs.

## What's new in this fork

### Screenshot base64 passthrough

Return the screenshot as a base64 string directly in the JSON response — no file path round-trip needed. Pass the image straight into a chat UI (e.g. Openclaw) without touching the filesystem.

```bash
# JSON API
{ "action": "screenshot", "returnBase64": true }
# Response includes: { "path": "...", "base64": "iVBORw0KGgo..." }
```

### Cursor position overlay on screenshots

After any `click` or `hover`, take a screenshot with `cursor: true` to draw a red crosshair at the exact position the action landed. Makes it easy to visually confirm the AI clicked the right element.

```bash
# JSON API — after a click or hover:
{ "action": "screenshot", "cursor": true, "returnBase64": true }
```

### Page state (single-call full context)

Get the current URL, page title, accessibility snapshot, and a base64 screenshot all in one atomic call. Saves 3–4 round trips for AI agents that need the full picture.

```bash
# JSON API
{ "action": "page-state" }
# Response: { "url": "...", "title": "...", "snapshot": "...", "screenshot": "..." }

# Optional params
{ "action": "page-state", "interactive": true, "compact": false }
```

### Form state extraction

Dump all form fields on the page (or inside a specific form) as structured JSON — name, id, type, current value, checked state, disabled, required, placeholder.

```bash
# JSON API — all fields on page
{ "action": "form-state" }

# Scoped to a specific form
{ "action": "form-state", "selector": "form#login" }

# Response: { "fields": [{ "tag": "input", "type": "email", "name": "email", "value": "...", ... }] }
```

### WebSocket message inspection

Capture and inspect WebSocket frames sent and received by the page. Essential for real-time apps (chat, trading dashboards, live feeds) where HTTP request tracking is blind.

```bash
# JSON API — enable capture and list messages
{ "action": "websocket-messages" }

# Filter by direction, limit results
{ "action": "websocket-messages", "direction": "received", "limit": 50 }

# Clear buffer
{ "action": "websocket-clear" }

# Response: { "messages": [{ "url": "wss://...", "direction": "received", "payload": "...", "timestamp": 1234567890, "opcode": 1 }], "total": 42 }
```
