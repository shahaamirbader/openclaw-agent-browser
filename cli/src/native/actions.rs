...
pub struct DaemonState {
    ...
    /// Last position (CSS pixels) where a click or hover action was performed.
    pub last_cursor_pos: Option<(f64, f64)>,
    /// Captured WebSocket messages (ring buffer, max 500).
    pub ws_messages: Vec<WsMessage>,
    /// Whether WebSocket capture is enabled.
    pub ws_message_capture: bool,
    /// Maps CDP requestId → WebSocket URL (populated by Network.webSocketCreated).
    pub ws_request_urls: HashMap<String, String>,
...
impl DaemonState {
    pub fn new() -> Self {
        Self {
            ...
            ws_messages: Vec::new(),
            ws_message_capture: false,
            ws_request_urls: HashMap::new(),
            pending_dialog: None,
            stream_client: None,
            stream_server: None,
        }
            ...
            match rx.try_recv() {
                Ok(event) => {
                    ...
                    match event.method.as_str() {
                        ...
                        "Network.webSocketCreated" if self.ws_message_capture => {
                            if let (Some(rid), Some(url)) = (
                                event.params.get("requestId").and_then(|v| v.as_str()),
                                event.params.get("url").and_then(|v| v.as_str()),
                            ) {
                                self.ws_request_urls.insert(rid.to_string(), url.to_string());
                            }
                        }
                        "Network.webSocketFrameReceived" if self.ws_message_capture => {
                            let ts = std::time::SystemTime::now()
                                .duration_since(std::time::UNIX_EPOCH)
                                .map(|d| d.as_millis() as u64)
                                .unwrap_or(0);
                            let rid = event.params.get("requestId").and_then(|v| v.as_str()).unwrap_or("").to_string();
                            let url = self.ws_request_urls.get(&rid).cloned().unwrap_or_default();
                            let payload = event.params.get("response").and_then(|r| r.get("payloadData")).and_then(|v| v.as_str()).unwrap_or("").to_string();
                            let opcode = event.params.get("response").and_then(|r| r.get("opcode")).and_then(|v| v.as_i64()).unwrap_or(1);
                            if self.ws_messages.len() >= 500 {
                                self.ws_messages.remove(0);
                            }
                            self.ws_messages.push(WsMessage { url, direction: "received".to_string(), payload, timestamp_ms: ts, opcode });
                        }
                        "Network.webSocketFrameSent" if self.ws_message_capture => {
                            let ts = std::time::SystemTime::now()
                                .duration_since(std::time::UNIX_EPOCH)
                                .map(|d| d.as_millis() as u64)
                                .unwrap_or(0);
                            ...
                            self.ws_messages.push(WsMessage { url, direction: "sent".to_string(), payload, timestamp_ms: ts, opcode });
                        }
                        _ => {}
                    }
                }
...
pub async fn execute_command(cmd: &Value, state: &mut DaemonState) -> Value {
    ...
    let result = match action {
        ...
        "page-state" => handle_page_state(cmd, state).await,
        "form-state" => handle_form_state(cmd, state).await,
        "websocket-messages" => handle_websocket_messages(cmd, state).await,
        "websocket-clear" => handle_websocket_clear(state).await,
...
/// Extracts all form field values from the page (or an optional root selector) as JSON.
async fn handle_form_state(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
    let mgr = state.browser.as_ref().ok_or("Browser not launched")?;
                ...
                return Array.from(root.querySelectorAll('input, select, textarea')).map(function(el) {{
                    return {{
                        ...
                        value: el.tagName.toLowerCase() === 'select'
                            ? Array.from(el.selectedOptions).map(function(o) {{ return o.value; }}).join(',')
                            : (el.value || null),
...
/// Returns URL, title, accessibility snapshot, and a base64 screenshot in a single call.
async fn handle_page_state(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    let mgr = state.browser.as_ref().ok_or("Browser not launched")?;
    let session_id = mgr.active_session_id()?.to_string();
    ...
    let screenshot_b64 = screenshot::take_screenshot(
        &mgr.client,
        &session_id,
        &state.ref_map,
        &ScreenshotOptions {
...
async fn handle_screenshot(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let options = ScreenshotOptions {
        ...
        output_dir: cmd
            ...
            .and_then(|v| v.as_str())
            .map(String::from),
        cursor_pos: if draw_cursor { state.last_cursor_pos } else { None },
    };
    ...
    if return_base64 {
        response["base64"] = json!(result.base64);
    }
...
async fn handle_click(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    .await?;
    state.last_cursor_pos = Some((cx, cy));
...
// ---------------------------------------------------------------------------
// WebSocket inspection handlers
// ---------------------------------------------------------------------------

async fn handle_websocket_messages(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    if !state.ws_message_capture {
        state.ws_message_capture = true;
        if let Some(ref mgr) = state.browser {
            if let Ok(session_id) = mgr.active_session_id() {
                let _ = mgr
                    .client
    ...
    let direction_filter = cmd.get("direction").and_then(|v| v.as_str()).map(String::from);

    let total = state.ws_messages.len();
    let messages: Vec<Value> = state
        .ws_messages
        .iter()
        .rev()
        .filter(|m| direction_filter.as_deref().map_or(true, |d| m.direction == d))
        .take(limit)
        .map(|m| json!({
            "url": m.url,
...
async fn handle_websocket_clear(state: &mut DaemonState) -> Result<Value, String> {
    let count = state.ws_messages.len();
