...
pub struct ScreenshotOptions {
    ...
    /// If set, draw a red crosshair at this position (CSS pixels) on the screenshot.
...
pub async fn take_screenshot(
    ...
    if let Some((cx, cy)) = options.cursor_pos {
        base64 = draw_cursor_overlay(&base64, cx, cy).unwrap_or(base64);
    }
...
/// Draws a red crosshair at `(cx, cy)` on the base64-encoded PNG and returns
/// a new base64-encoded PNG. Falls back silently to the original if anything fails.
fn draw_cursor_overlay(base64_data: &str, cx: f64, cy: f64) -> Result<String, String> {
    let bytes = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, base64_data)
        .map_err(|e| format!("cursor overlay decode: {}", e))?;
    let mut img = image::load_from_memory(&bytes)
        .map_err(|e| format!("cursor overlay load: {}", e))?
        .into_rgba8();
    let (w, h) = (img.width() as i64, img.height() as i64);
    let cx = cx.round() as i64;
    let cy = cy.round() as i64;
    let red = image::Rgba([220u8, 38, 38, 230]);
    let arm: i64 = 14;
    let thick: i64 = 2;
    for dx in -arm..=arm {
        for dt in -thick..=thick {
            let (px, py) = (cx + dx, cy + dt);
    ...
    img.write_to(&mut buf, image::ImageFormat::Png)
        .map_err(|e| format!("cursor overlay encode: {}", e))?;
